...
export class DashboardController {
	...
	async getTotalBySchoolType() {
		...
		return TransformResponse.map(
			data,
			'Datos de tipo de escuela obtenidos con éxito',
		)
	}
	...
	async getTotalByGrade() {
		...
		return TransformResponse.map(
			data,
			'Datos de grados académicos obtenidos con éxito',
		)
	}
	...
	async getTotalByReferenceType() {
		...
		return TransformResponse.map(
			data,
			'Datos de tipos de referidos obtenidos con éxito',
		)
	}
	...
	async getTotalByContactMedium() {
		...
		return TransformResponse.map(
			data,
			'Datos por medio de contacto obtenidos con éxito',
		)
	}
	...
	async getPromoterTotalByContactType(@Request() req) {
		const userId = req.user.id
		const data =
			await this.dashboardService.getPromoterTotalByContactType(userId)
		return TransformResponse.map(
			data,
			'Datos de medio de contacto obtenidos con éxito',
		)
	}
	...
	async getPromoterTotalBySchoolType(@Request() req) {
		const userId = req.user.id
		const data =
			await this.dashboardService.getPromoterTotalBySchoolType(userId)
		return TransformResponse.map(
			data,
			'Datos de tipo de escuela obtenidos con éxito',
		)
	...
	async getPromoterTotalByScholarship(@Request() req) {
		const userId = req.user.id // Usuario autenticado
		const data =
			await this.dashboardService.getPromoterTotalByScholarship(userId)
		return TransformResponse.map(data, 'Datos de becas obtenidos con éxito')
	}
	...
	async getPromoterTotalByFollowUp(@Request() req) {
		...
		return TransformResponse.map(
			data,
			'Datos de seguimiento obtenidos con éxito',
		)
	}
}
