...
private async checkPhonesAreUnique(phoneArray: string[], leadIdToExclude?: string) {
...
private async checkEmailsAreUnique(emailArray: string[], leadIdToExclude?: string){
...
async findAll(params: FilterLeadDto) {
    ...
    async findOne(id: string, params = { 'with-timeline': false, comments: false }) {
        ...
        if (!data) {
          throw new HttpException(`Lead not found with id ${id}`, HttpStatus.NOT_FOUND);
        }
...
async CreateFromFileShare(file: Express.Multer.File) {
  ...
  for (const row of data) {
    ...
    if (emailArray.length === 0 && phoneArray.length === 0) {
      console.warn(`Lead omitido: sin teléfono ni correo (${JSON.stringify(row)})`);
      continue;
    }
    ...
    try {
      ...
      const lead = await this.leads.create({
        data: {
          ...row,
          emails: emailArray.length > 0 ? {
            createMany: {
              data: emailArray.map((email) => ({ email })),
            },
          } : undefined,
          phones: phoneArray.length > 0 ? {
            createMany: {
      ...
      console.warn(`Lead omitido por duplicado (${email || phone}): ${error.message}`);
    }
  }
  ...
  return TransformResponse.map(
    ...
    async getFilterValues(filter: string) {
        switch (filter) {
            ...
            case 'followUp':
                ...
                return followUps.map(followUp => ({ id: followUp.id, name: followUp.name }));
            ...
            case 'countryId':
                ...
                return countries.map(country => ({ id: country.id, name: country.name }));
            ...
            case 'userId':
                const users = await this.user.findMany({
                    select: { id: true, name: true, paternalSurname: true, maternalSurname: true },
                });
    ...
    async getFilteredLeads(params: LeadsFilterDto) {
        const where: any = {
          ...
          ...(params.careerId && { information: { careerInterest: params.careerId } }),
          ...(params.countryId && { city: { state: { countryId: params.countryId } } }),
          ...
          ...(params.followUp && { information: { followUp: { name: params.followUp } } }), // Filtrado por nombre del seguimiento
...
async getFilteredLeadsByUser(params: LeadsFilterDto, user: any) {
    const where: any = {
        ...
        ...(params.campaignId && { campaign: { id: params.campaignId } }),
        ...(params.promoterId && { promoter: { id: params.promoterId } }),
        ...(params.followUp && { information: { followUp: { name: params.followUp } } }),
