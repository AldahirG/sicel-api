...
export class CyclesService extends PrismaClient implements OnModuleInit {
  ...
  async create(createCycleDto: CreateCycleDto) {
    ...
    return TransformResponse.map(data, 'Ciclo escolar creado con éxito!!', 'POST', HttpStatus.CREATED)
  }
  ...
  async update(id: string, updateCycleDto: UpdateCycleDto) {
    ...
    return TransformResponse.map(data, 'Ciclo escolar actualizado con éxito!!', 'PUT');
  }
  ...
  async remove(id: string) {
    ...
    return TransformResponse.map(data, 'Ciclo escolar eliminada con éxito!!', 'DELETE')
  }
