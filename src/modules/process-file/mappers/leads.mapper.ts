...
export class CsvLeadsResource {
	static map(lead: ICsvDto): CsvInterface {
		const information = lead.Nombre
			? { create: { name: lead.Nombre } }
			: undefined
		const emails = lead.Correos
			? {
					createMany: {
		...
		return {
			...
			createAt:
				lead?.Created_at !== '' ? new Date(lead?.Created_at) : undefined,
			grade: lead?.Grade,
