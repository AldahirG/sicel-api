...
router.post('/aset-name', async (req, res) => {
    try {
        ...
        if (existingAsetName) {
            return res.status(400).json({ mensaje: 'Aset Name con este nombre y medio de contacto ya existe.' });
        }
...
router.put('/aset-name/:id', async (req, res) => {
    try {
        ...
        if (existingAtrributes) {
            return res.status(400).json({ error: 'Ya existe un aset-name con este nombre y/o medio de contacto.' });
        }
...
router.get('/aset-names/list', async (req, res) => {
    try {
        ...
        console.error('Error al encontrar los Aset Name:', error);
        res.status(500).send('Error interno del servidor');
    }
});
